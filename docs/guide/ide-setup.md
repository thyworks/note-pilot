...
Claude Code の使用には **Claude.ai の有料プラン**が必要です。

::: warning 課金が必要です
Claude Code は無料では利用できません。下記いずれかのプランへの加入が必要です。
:::

| プラン | 月額（USD） | 向いている人 |
|-------|------------|-------------|
| **Pro** | $20 / 月 | 個人・ライトユーザー（note-pilot ならこれで十分） |
| **Max** | $100 / 月 | 1日に何十本もの記事を生成するヘビーユーザー |

### まだ登録していない場合

1. [https://claude.ai/](https://claude.ai/) にアクセスしてアカウントを作成
2. 右上のアカウントアイコン → **「Upgrade」** をクリック
3. プランを選択して支払い情報を入力
4. サブスクリプション開始後、VS Code の拡張機能からログインできるようになります

::: tip どちらを選べばいい？
note-pilot を日常的に使うなら **Pro ($20/月)** で十分です。
:::
...
1. VS Code で Claude Code を開く（左サイドバーの **✦ アイコン** をクリック）
2. **「Sign in to Claude.ai」** をクリック
3. ブラウザが開き、Claude.ai の認証ページが表示される
4. Claude.ai にログインして **「Authorize」** をクリック
5. VS Code に戻ると認証完了 — Claude Code が使えるようになります
